...
int main(int argc, char *argv[])
{
   ...
   /* todo: print ^^ to buffer and then call dout(), or just use python */
...
void ShutDown()
{
  ...
  //   cputs(data[END_TEXT].dat);
	...
	if(debug) {
		dout("Last Debugging OUTput message; Closing this file!\n");
		fclose(doutput);
		printf("\ndout() messages logged: %d.\n",dcount);
	}
}//ShutDown


void dout(char bufd[]) // forces to un-DRY the code, C fails.
{
  //const int padding = 10; // ten digts make me great horn - Ken Kandle ;)
	if(debug) {
	   dcount++;
	   fprintf(doutput, "(%0.3d):%s\n", dcount, bufd);
	}
}
