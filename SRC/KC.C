/* kc.c; July 2001 re-release with minor fixes (and same major flaws!)

		KC, a little project by
		Keinall H. Caddle.

	Please see readme.html for user details, build.html for developer
	friendly (or, at least, luser unfriendly) stuff.
  ...
  if (!data) {// no data, no game.
   ...
   set_gfx_mode(GFX_AUTODETECT, w, h, 0, 0);
	if (!dullstart) { // a nice logo would be neater here, maybe.
		clear(screen);
		int offset = 16;// note the C++ style variable initalisation!
   ...
   set_pallete((const PALETTE)data[PAL_GAME].dat);
  ...
  do
  {
    ...
    if (( sel_st > 0)&& (AbortNow == FALSE)) {
      ...
      if ((type!=Imp)&& (suicide==FALSE) &&(type < EatUp)) {
        ...
        // textprintf (s, font,  BoardX, 32, LineColor, "Invalid First move.Rejected!");
        // UpdateScreen(); rest(1000*2);
...
void SecondMove()
{
  ...
  do  {
    ...
    if (( sel_st > 0)&& (AbortNow == FALSE)) {
      //textprintf (s, font,  BoardX, 32, LineColor, "Move 2nd Piece into Inner Court.");
...
//			char sstr[100] = "[Screenshot Message!]";
	...
	while (sshot_rq()) ;	{
		; // do nothing... timeout should be here, though
	}
